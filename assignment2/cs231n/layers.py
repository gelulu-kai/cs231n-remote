...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (gamma, x, sample_mean, sample_var, eps, x_normed)
...
def batchnorm_backward(dout, cache):
    ...
    pass
    gamma, x, u_b, sigma_squared_b, eps, x_normed = cache
    N = x.shape[0]
...
def batchnorm_backward_alt(dout, cache):
    ...
    gamma, x, u_b, sigma_squared_b, eps, x_normed = cache
    N = x.shape[0]
    dgamma = np.sum(x_normed * dout, keepdims=True, axis=0)
    dbeta = np.sum(dout, keepdims=True, axis=0)
    dx_normed = dout*gamma
    dvar = np.sum(dx_normed * (x - u_b) * (-0.5) * (sigma_squared_b + eps)**(-1.5), keepdims=True, axis=0)
    dmean = np.sum(-1.0 * dx_normed*(sigma_squared_b + eps)**(-0.5), axis=0) + dvar * (-2) * np.mean(x - u_b, keepdims=True, axis=0)
    dx = dmean * 1.0 / N + dvar * 2.0 * (x - u_b) / N + dx_normed / np.sqrt(sigma_squared_b + eps)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    dbeta = np.sum(dout, keepdims=True, axis=0)
    dx_normed = dout*gamma
    dvar = np.sum(dx_normed * (x - u_b) * (-0.5) * (sigma_squared_b + eps)**(-1.5), keepdims=True, axis=1)
    dmean = np.sum(-1.0 * dx_normed*(sigma_squared_b + eps)**(-0.5), axis=1, keepdims=True) + dvar * (-2) * np.mean(x - u_b, keepdims=True, axis=1)
    dx = dmean * 1.0 / D + dvar * 2.0 * (x - u_b) / D + dx_normed / np.sqrt(sigma_squared_b + eps)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_out):
      for j in range(W_out):
        x_pad_mask = x_pad[:, :, i*stride:i*stride + HH, j*stride:j*stride + WW]#(N,C,HH,WW)
        for k in range(F):
          out[:, k, i, j] = np.sum(x_pad_mask * w[k, :, :, :], axis=(1,2,3))
    out = out + (b)[np.newaxis, :, None, None]#(N,F,H,W)
    ...
    for i in range(H_out):
      for j in range(W_out):
          ...
          for k in range(F): #compute dw
              dw[k ,: ,: ,:] += np.sum(x_pad_masked * (dout[:, k, i, j])[:, None, None, None], axis=0)#dout(N,F,H,W)dw(F,C,HH,WW)
          for n in range(N): #compute dx_pad
              dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += np.sum((w[:, :, :, :] * 
                                                  (dout[n, :, i, j])[:,None ,None, None]), axis=0)
    dx = dx_pad[:,:,pad:-pad,pad:-pad]#crop
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
      for j in range(W_out):
        ...
        x_mask_bool = x_mask == (x_mask_max)[:, :, None, None]
        dx[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] = (dout[:, :, i, j])[:, :, None, None] * x_mask_bool
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out1, cache = batchnorm_forward(x.reshape(N*H*W, C), gamma, beta, bn_param)
