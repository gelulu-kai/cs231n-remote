...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2, dw2, db2 = affine_relu_backward(dscore, cache2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for lay in range(self.num_layers - 1):
          if self.normalization != None:
            if self.normalization == 'batchnorm':
              layer_input, fw_cache[lay] = affine_bn_relu_forward(layer_input, 
                                          self.params['W%d'%(lay+1)], self.params['b%d'%(lay+1)], 
                                          self.params['gamma%d'%(lay+1)], self.params['beta%d'%(lay+1)], self.bn_params[lay])
            if self.normalization == 'layernorm':
              layer_input, fw_cache[lay] = affine_ln_relu_forward(layer_input, 
                                          self.params['W%d'%(lay+1)], self.params['b%d'%(lay+1)], 
                                          self.params['gamma%d'%(lay+1)], self.params['beta%d'%(lay+1)], self.bn_params[lay])
          else:
            layer_input, fw_cache[lay] = affine_relu_forward(layer_input, self.params['W%d'%(lay + 1)], self.params['b%d'%(lay+1)])

          if self.use_dropout:
            layer_input, drop_cache[lay] = dropout_forward(layer_input, self.dropout_param)
        scores, fw_cache[self.num_layers] = affine_forward(layer_input, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
        ...
        dx, dw, db = affine_backward(dscore, fw_cache[self.num_layers])
        ...
        for idx in range(self.num_layers - 1):
          ...
          if self.use_dropout:
            dhout = dropout_backward(dhout, drop_cache[lay])
          if self.normalization == 'batchnorm':
            dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dhout, fw_cache[lay])
          elif self.normalization == 'layernorm':
            dx, dw, db, dgamma, dbeta = affine_ln_relu_backward(dhout, fw_cache[lay])
          else:
            dx, dw, db = affine_relu_backward(dhout, fw_cache[lay])
