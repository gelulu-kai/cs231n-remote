...
def batchnorm_backward_alt(dout, cache):
    ...
    gamma, x, u_b, sigma_squared_b, eps, x_normed = cache
    N = x.shape[0]
    dgamma = np.sum(x_normed * dout)
    dbeta = np.sum(dout)
    dx_normed = dout*gamma
    dvar = np.sum(dx_normed * (x - u_b) * (-0.5) * np.power(sigma_squared_b + eps, -1.5), keepdims=True)
    dmean = np.sum(-dx_normed*(sigma_squared_b + eps)**(-0.5), keepdims=True) + dvar * (-2) * np.mean(x - u_b, keepdims=True)
    dx = dmean / N + dvar * 2 * np.mean(x - u_b, keepdims=True) + dx_normed / np.sqrt(sigma_squared_b + eps)
