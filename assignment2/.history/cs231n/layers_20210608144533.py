...
def batchnorm_backward_alt(dout, cache):
    ...
    dmean = np.sum(-dx_normed*(sigma_squared_b + eps)**(-0.5), keepdims=True)
    dx = dmean / N + dvar * 2 * (np.sum(x - u_b, keepdims=True) - dmean * np.sum(x-u_b, keepdims=True)) / N + dx_normed / np.sqrt(sigma_squared_b + eps)
