...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores = np.dot(hidden_layer, W2) + b2  #scores.shape = N,C
        ...
        print('f',f.shape)
        loss = np.log(np.exp(f)).sum(axis=1) - f[range(N),y].sum()
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            # pass
            index = np.random.choice(num_train, batch_size, replace=True)#可以取相同的数
            X_batch = X[index]
            y_batch = y[index]
    ...
    def predict(self, X):
        ...
        pass
        hiddden_layer = np.maximum(0, np.dot(X, self.params['W1']) + self.params['b1'])

        scores = np.dot(hiddden_layer, self.params['W2']) + self.params['b2']  #scores.shape = N,C
