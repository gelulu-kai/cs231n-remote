...
def layernorm_backward(dout, cache):
    ...
    dmean = np.sum(dx_hat*-1 / np.sqrt(sample_var + eps), axis=0) + dvar*np.mean(-2*(x - sample_mean), axis=1, keepdims=True)
    dx = dx_hat / np.sqrt(sample_var + eps) + dmean * 1.0 / N + dvar*2.0*(x - sample_mean) / N
    dgamma = np.sum(x_hat*dout, axis=0)
    dbeta = np.sum(dout, axis=0)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    pass
    N, C, H, W = dout.shape
    dx_temp, dgamma, dbeta = batchnorm_backward_alt(dout.transpose(0, 3, 2, 1).reshape(N*W*H, C), cache)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_groupnorm*-0.5*(x_group - mean)*np.power(var +eps, -1.5), axis=(2,3,4), keepdims=True)
    #dmean
    N_group = C//G*H*W
    ...
    #dx_group
    dx_group1 = np.sum(dx_groupnorm/np.sqrt(var + eps), axis=(2,3,4), keepdims=True)
