...
def layernorm_backward(dout, cache):
    ...
    dmean = np.sum(dx_hat*-1 / np.sqrt(sample_var + eps), axis=1, keepdims=True) + dvar*np.mean(-2*(x - sample_mean), axis=1, keepdims=True)
    dx = dx_hat / np.sqrt(sample_var + eps) + dmean * 1.0 / D + dvar*2.0*(x - sample_mean) / D

    dgamma = Dp.sum(x_hat*dout, axis=0)
    dbeta = np.sum(dout, axis=0)
